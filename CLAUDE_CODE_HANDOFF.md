...
**아키텍처 이전 여부** — "사이트가 무겁다"는 지적에 대해 3단계 안을 제시했었음:
- **A안(추천, 이번에 일부 적용함)**: 지금 구조(Apps Script+HtmlService 단일 파일) 유지하면서 최적화만. 백그라운드 폴링 중지는 이미 적용. 추가로 할 수 있는 것: 솔로 모듈 지연 로딩, 배포 전 minify 등.
- **B안(하이브리드)**: 스프레드시트+Apps Script를 API 서버로만 쓰고, 프론트는 Vercel/GitHub Pages 같은 정적 호스팅으로 분리.
- **C안(완전 재작성)**: Firestore/Supabase + 서버리스 함수 + React/Vue SPA로 완전히 새로 만들기.

소규모 개인/고정 파티용이라는 원래 스코프를 감안하면 A안을 추천했고, 이번에 A안의 일부(백그라운드 폴링 정지)를 실제로 적용했습니다. B/C안으로 갈지, A안을 더 밀어붙일지(지연 로딩 등)는 사용자 결정 대기 중입니다. **클로드 코드에서 이어받으면 가장 먼저 이 방향을 사용자에게 다시 확인하는 게 좋습니다.**
